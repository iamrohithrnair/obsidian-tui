...
trusted publishing, which needs no secret — but it can only be configured on
a package that already exists, so the first release of each of the five
packages has to go out by hand.
